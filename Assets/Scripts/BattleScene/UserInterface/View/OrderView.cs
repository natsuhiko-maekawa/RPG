        ...
        public async Task StartAnimation(List<OrderViewDto> dtoList)
        {
            ...
            foreach (var (dto, i) in dtoList.Select((x, i) => (x, i)))
            {
                ...
                _imageList[i].sprite = dto.ItemType switch
                {
                    ...
                    ItemType.Enemy when dto.EnemyName != null => _spriteFlyweight.Get(dto.EnemyName),
                    ItemType.Ailments when dto.AilmentsInt != null => _ailmentsIconArray[(int)dto.AilmentsInt],
