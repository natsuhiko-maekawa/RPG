...
namespace BattleScene.Framework.Views
{
    ...
    public class SelectSkillView : MonoBehaviour
    {
        ...
        public Task StartAnimation(SelectSkillViewModel model)
        {
            _window.enabled = true;
            _rightArrow.enabled = true;
            _rightArrow.rectTransform.localPosition = new Vector3(-147, -model.HighlightRow * SlotHeight + 69, 0);
            _upArrow.enabled = model.ViewUpArrow;
            _downArrow.enabled = model.ViewDownArrow;
            SetText(model.SkillList.Count);
            foreach (var (skill, index) in model.SkillList.Select((x, i) => (x, i)))
            {
                _textList[index].text = skill.Name;
                if (skill.Disabled)
                    _textList[index].color = Color.gray;
                else if (index == model.HighlightRow)
